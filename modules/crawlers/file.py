...
# Define FileExtractionError if not already defined in errors.py
class FileExtractionError(CrawlerError):
    """Error when file extraction fails."""
    pass
...
class FileCrawler:
    ...
    async def _find_matching_files(self, query: str) -> List[str]:
        ...
        # Normalize query: remove punctuation and convert to lowercase
        import re
            ...
            try:
                for root, _, files in os.walk(base_dir):
                    for file in files:
                        ...
                        # Normalize filename: remove file extension, remove punctuation
                        file_base = os.path.splitext(file)[0].lower()
                        normalized_file = re.sub(r'[^\w\s]', '', file_base)
                ...
                logger.error(f"Error searching directory {base_dir}: {str(e)}")
